...
def midi_to_str_array(mid):
  ...
  return correct_time_notes


all_notes = []
song_indeces = [] #at which index the song begins, since we have one huge file with many songs togethr
current_offset = 0 #
for i in range(4):
  midi = MidiFile(sys.argv[i + 1])
  notes = midi_to_str_array(midi)
  all_notes += notes
  song_indeces.append(current_offset)
